...
class ETH_Base extends CrosschainBase {
  ...
  storemanInfo(opts, skipValidation) {
    ...
    return new Promise((resolve, reject) => {
      action.then(res => {
        const storemanInfo = this.parseOutput('StoremanGroupAdmin_ETH', 'mapCoinSmgInfo', res);
  ...
  buildStoremanInfoTx(opts, skipValidation) {
    ...
    const to = this.config.addresses.StoremanGroupAdmin_ETH;
  ...
  buildStoremanInfoData(opts, skipValidation) {
    ...
    const { storeman } = opts;
    const { mapCoinSmgInfo } = this.config.signatures.StoremanGroupAdmin_ETH;
